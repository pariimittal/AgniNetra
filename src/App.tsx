...
import { getEvents, getFacilities } from './api/client'
      ...
      {loading ? (
        ...
        <div className="grid gap-5 xl:grid-cols-[1.55fr_0.7fr]">
          <div className="command-map-panel tech-panel rounded border border-slate-800 bg-[#0d1218] p-3">
            <MapView events={events} selectedEventId={selectedEventId} onSelectEvent={setSelectedEventId} />
          </div>
...
function MapView({ events, selectedEventId, onSelectEvent }: { events: ThermalEvent[]; selectedEventId: string; onSelectEvent: (id: string) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="map-shell relative h-[560px] overflow-hidden rounded border border-slate-800">
        <MapLeaflet events={events} selectedEvent={selectedEvent} onSelect={onSelectEvent} />
        <div className="map-overlay pointer-events-none absolute left-4 top-4 z-[500] rounded border border-cyan-300/20 bg-slate-950/70 px-3 py-2 backdrop-blur-md">
          ...
          <div className="mt-2 flex gap-3 text-[10px] uppercase tracking-[0.16em] text-slate-300">
            <span>{events.length} events</span><span>{facilities.length} facilities</span>
          </div>
        </div>
...
function MapLeaflet({ events, selectedEvent, onSelect }: { events: ThermalEvent[]; selectedEvent: ThermalEvent | null; onSelect: (id: string) => void }) {
    ...
    <div>
      ...
      <div className="mb-5 rounded border border-slate-800 bg-[#0d1218] p-4">
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6 lg:gap-3">
          ...
          <select value={filters.facility} onChange={(event) => setFilters((prev) => ({ ...prev, facility: event.target.value }))} className="rounded border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-100"><option value="ALL">All facilities</option>{facilities.map((facility) => <option key={facility.id} value={facility.id}>{facility.name}</option>)}</select>
        ...
        <div className="space-y-5">
          ...
          <div className="rounded border border-slate-800 bg-[#0d1218] p-5">
            ...
            <div className="mt-3 space-y-2 text-sm text-slate-300">
              ...
              <div>Baseline: {event.facilityId ? facilities.find((item) => item.id === event.facilityId)?.baselineBrightness ?? 0 : 0} K</div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <div className="rounded border border-slate-800 bg-[#0d1218] p-5">
          ...
          <div className="mt-4 space-y-3">
            {(facilityHistory[facility.id] ?? []).slice(0, 3).map((entry) => (
